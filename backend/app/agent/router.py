...
{{
  ...
  "current_year": "2026"
    ...
    def _extract_location_fallback(question: str) -> str:
        ...
        q = re.sub(r'^(compare|comparison|show\s+me|find|where\s+is|what\s+is\s+the|what\s+are\s+the|analyze|inspect|view)\s+', '', q, flags=re.IGNORECASE)
        ...
        q = re.sub(r'^(for|around|in|near|at|over|of)\s+', '', q, flags=re.IGNORECASE)
    ...
    @staticmethod
    def interpret(question: str, location_hint: Optional[str] = None) -> Dict[str, Any]:
        clean_q = question.strip().lower()

        # If an explicit location name was passed from the UI/voice state, prioritize it directly
        if location_hint and len(location_hint.strip()) > 2:
            loc_search_str = location_hint.strip()
            ...
            if llm_norm and llm_norm.get("location_query"):
                ...
                loc_search_str = QueryInterpreter._extract_location_fallback(question)
                corrected_question = question
                is_comp = any(w in clean_q for w in [
                    "change", "changes", "compare", "difference", "differ", "before vs after", "vs",
                    "historical", "reclamation", "expansion", "growth", "shrink", "deforestation"
        ...
        # 2. Second Pass: Dynamic Geocoding using Normalized Location Query
        emit_log("INFO", "GEOCODER", f"Resolving coordinates for normalized place: '{loc_search_str}'...")
        ...
        return {
            ...
            "needs_grounding": task in ["grounding", "change_analysis"] or any(w in clean_q for w in ["detail", "where", "highlight", "box", "show me"]),
