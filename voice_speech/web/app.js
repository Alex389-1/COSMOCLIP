...
function handleServerEvent(msg) {
  if (msg.type === 'barge_in' || msg.type === 'interrupted') {
    ...
    if (msg.name === 'compare_satellite_images') {
      showToast(`🛰️ Bi-Temporal Change Detected for ${loc || 'location'}!`);
      setCockpitMode('change');
      const titleEl = document.getElementById('activeLocationTitle');
      const t1Img = document.getElementById('changeT1Img');
      const t2Img = document.getElementById('changeT2Img');
      const diffImg = document.getElementById('changeDiffImg');
      const summaryEl = document.getElementById('changeSummaryText');
      const t1Header = document.getElementById('t1Header');
      const t2Header = document.getElementById('t2Header');

      if (titleEl && loc) titleEl.innerText = `⏱️ ${loc} (Bi-Temporal Comparison)`;
      if (t1Header && args.date1) t1Header.innerText = `📅 Past Scene (T1: ${args.date1})`;
      if (t2Header && args.date2) t2Header.innerText = `📅 Present Scene (T2: ${args.date2})`;

      setTimeout(() => {
        if (t1Img) t1Img.src = `/session/default/change/t1?t=${Date.now()}`;
        if (t2Img) t2Img.src = `/session/default/change/t2?t=${Date.now()}`;
        if (diffImg) diffImg.src = `/session/default/change/diff?t=${Date.now()}`;
      }, 300);

      if (summaryEl && msg.result) summaryEl.innerText = msg.result;

    } else if (msg.name === 'analyze_satellite_image' || msg.name === 'get_location_coordinates') {
      showToast(`🛰️ Live Satellite Analysis for ${loc || 'current location'}...`);
      setCockpitMode('single');
      const titleEl = document.getElementById('activeLocationTitle');
      const mainImg = document.getElementById('mainSatelliteImg');
      if (titleEl && loc) titleEl.innerText = `${loc}`;

      setTimeout(() => {
        if (mainImg) mainImg.src = `/session/default/image?t=${Date.now()}`;
      }, 300);

      updateTraceTelemetry({
        tool: msg.name,
        question: args.question || 'Satellite VQA Query',
        result: msg.result,
        latency: '~1200 ms',
      });
    }
...
// Satellite Image Modal & Telemetry Controls
const satModal = document.getElementById('satModal');
const activePreviewImg = document.getElementById('activePreviewImg');
const activeImageLabel = document.getElementById('activeImageLabel');
const satBadgeText = document.getElementById('satBadgeText');

function toggleSatModal() {
  if (!satModal) return;
  satModal.classList.toggle('show');
}
...
async function handleFileUpload(files) {
  ...
  try {
    const res = await fetch('/session/default/image', {
      method: 'POST',
      body: formData,
    });
    if (!res.ok) {
    ...
    showToast('Satellite image loaded & bound to session!');
    if (activePreviewImg) {
      activePreviewImg.src = `/session/default/image?t=${Date.now()}`;
    }
    if (activeImageLabel) {
      activeImageLabel.innerText = file.name;
    }
    if (satBadgeText) {
      satBadgeText.innerText = `Tile: ${file.name.slice(0, 14)}...`;
    }
  } catch (err) {
...
async function loadSampleTile(tileType) {
  showToast(`Loading Sentinel-2 ${tileType} tile...`);
  // Generate sample client-side canvas tile and upload
  ...
  if (tileType === 'water') {
    ctx.fillStyle = '#143c8c';
    ctx.fillRect(0, 0, 256, 256);
    ctx.fillStyle = '#788c50';
    ctx.beginPath();
    ctx.moveTo(0, 180);
    ctx.lineTo(120, 160);
    ctx.lineTo(256, 200);
    ...
    ctx.lineTo(0, 256);
    ctx.fill();
  } else if (tileType === 'urban') {
    ctx.fillStyle = '#8c8782';
    ctx.fillRect(0, 0, 256, 256);
    ctx.strokeStyle = '#3c3c3c';
    ctx.lineWidth = 6;
    ctx.strokeRect(40, 40, 176, 176);
    ctx.fillStyle = '#b4463c';
    for (let x = 20; x < 240; x += 40) {
      for (let y = 20; y < 240; y += 40) {
        ctx.fillRect(x, y, 24, 24);
      }
    }
  } else if (tileType === 'vegetation') {
    ctx.fillStyle = '#23782d';
    ctx.fillRect(0, 0, 256, 256);
    ctx.strokeStyle = '#468c32';
    ctx.lineWidth = 4;
    ctx.strokeRect(20, 20, 216, 216);
  } else {
    ctx.fillStyle = '#194ba0';
    ctx.fillRect(0, 0, 256, 256);
    ctx.fillStyle = '#828282';
    ctx.fillRect(0, 100, 160, 40);
    ctx.fillStyle = '#ffffff';
    ctx.fillRect(180, 85, 30, 12);
  }

  canvas.toBlob(async (blob) => {
    const filename = `sentinel2_${tileType}_tile.png`;
...
function setCockpitMode(mode) {
  const singleCard = document.getElementById('cockpitSingleMode');
  const changeCard = document.getElementById('cockpitChangeMode');
  const btnSingle = document.getElementById('btnViewSingle');
  const btnChange = document.getElementById('btnViewChange');

  if (mode === 'change') {
    if (singleCard) singleCard.style.display = 'none';
    if (changeCard) changeCard.style.display = 'block';
...
async function fetchLiveLocation(locationName) {
  setCockpitMode('single');
  const titleEl = document.getElementById('activeLocationTitle');
  const coordsEl = document.getElementById('activeCoordsText');
  const imgEl = document.getElementById('mainSatelliteImg');
  const telemSource = document.getElementById('telemSource');

  if (titleEl) titleEl.innerText = `${locationName}`;
  if (coordsEl) coordsEl.innerText = `Acquiring live Sentinel-2 / ArcGIS optical tile...`;
  showToast(`🛰️ Geocoding & acquiring real satellite map for ${locationName}...`);

  try {
    const res = await fetch(`/api/fetch?location=${encodeURIComponent(locationName)}&session_id=default`);
    const data = await res.json();

    if (imgEl) imgEl.src = `/session/default/image?t=${Date.now()}`;
    if (coordsEl && data.latitude) {
      coordsEl.innerText = `${data.latitude.toFixed(5)}° N, ${data.longitude.toFixed(5)}° E · Date: ${data.scene_date || 'Recent'}`;
    }
    if (telemSource) telemSource.innerText = `ArcGIS / S2`;

    showToast(`Real satellite imagery loaded for ${locationName}!`);
  } catch (err) {
    console.error('Location fetch failed:', err);
    showToast(`Acquisition error: ${err.message}`);
  }
}

async function triggerTemporalCompare(location, date1, date2, question) {
  setCockpitMode('change');
  const titleEl = document.getElementById('activeLocationTitle');
  const coordsEl = document.getElementById('activeCoordsText');
  const t1Img = document.getElementById('changeT1Img');
  ...
  if (titleEl) titleEl.innerText = `⏱️ ${location} (Temporal Comparison)`;
  if (coordsEl) coordsEl.innerText = `Comparing ${date1} vs ${date2} · Real Optical Satellite Feed`;
  showToast(`🛰️ Analyzing Bi-Temporal Changes for ${location}...`);

  try {
    const res = await fetch(`/api/compare?location=${encodeURIComponent(location)}&date1=${encodeURIComponent(date1)}&date2=${encodeURIComponent(date2)}&question=${encodeURIComponent(question)}&session_id=default`);
    const data = await res.json();

    if (t1Img) t1Img.src = `${data.t1_url}&t=${Date.now()}`;
    if (t2Img) t2Img.src = `${data.t2_url}&t=${Date.now()}`;
    if (diffImg) diffImg.src = `${data.diff_url}&t=${Date.now()}`;
    if (summaryEl) summaryEl.innerText = data.summary;
    if (t1Header) t1Header.innerText = `📅 Past Scene (T1: ${date1})`;
    if (t2Header) t2Header.innerText = `📅 Present Scene (T2: ${date2})`;

    showToast(`Bi-temporal comparison complete for ${location}!`);
  } catch (err) {
...
function updateTraceTelemetry(info) {
  const traceTool = document.getElementById('traceTool');
  ...
  const traceSceneDate = document.getElementById('traceSceneDate');
  const traceSource = document.getElementById('traceSource');

  if (traceTool && info.tool) traceTool.innerText = info.tool;
  if (traceLatency && info.latency) traceLatency.innerText = info.latency;
  if (traceQuery && info.question) traceQuery.innerText = info.question;
  if (traceCoords && info.coords) traceCoords.innerText = info.coords;
  if (traceSceneDate && info.sceneDate) traceSceneDate.innerText = info.sceneDate;
  if (traceSource && info.source) traceSource.innerText = info.source;
}
